...
class MLP(nn.Module):
    ...
    def forward(self, x, dum_text_encoder):
        x = x[...,1024:]
        y =  self.out_ln(self.dropout(self.gelu(self.in_ln(x))))
        return y.mean((-3,-2))
...
if __name__ == "__main__":
    ...
    for split in range(10):
        ...
        maxvqa = MLP(768,64,1).cuda()
        #MaxVQA(text_tokens, embedding, text_encoder, share_ctx=True).to(device)
